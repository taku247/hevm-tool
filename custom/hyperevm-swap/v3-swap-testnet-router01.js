...
/**
 * HyperSwap V3 スワップ機能（SwapRouter01版）
 * ChatGPT検証済み: deadline必須、8パラメータ
 */
class HyperSwapV3Router01 {
  constructor() {
    ...
    this.config = {
      chainId: 998,
      swapRouter01: '0xD81F56576B1FF2f3Ef18e9Cc71Adaa42516fD990', // deadline必須版
  ...
  async ensureApproval(wallet, tokenSymbol, amount) {
    try {
      ...
      // 現在のAllowance確認
      const currentAllowance = await token.allowance(wallet.address, this.config.swapRouter01);
      ...
      const tx = await token.approve(this.config.swapRouter01, maxAmount);
  ...
  async swap(tokenInSymbol, tokenOutSymbol, amountIn, fee = null, slippagePercent = 0.5) {
    try {
      ...
      // 5. V3スワップ実行（Router01専用）
      console.log('\n🚀 V3スワップ実行 (SwapRouter01):');
      const swapRouter = new ethers.Contract(this.config.swapRouter01, this.swapRouterABI, wallet);
      
      const deadline = Math.floor(Date.now() / 1000) + 60 * 20; // ChatGPT修正: 未来の時刻確保
      ...
      const params = {
        ...
        deadline: deadline, // ChatGPT修正: Router01はdeadline必須
        amountIn: amountIn,
...
async function main() {
  ...
  if (args.includes('--help') || args.length === 0) {
    console.log(`
🔄 HyperSwap V3 スワップツール (SwapRouter01版)
...
使用方法:
  ...
  node custom/hyperevm-swap/v3-swap-testnet-router01.js --tokenIn HSPX --tokenOut WETH --amount 100 --fee 500
...
`);
  ...
  try {
    const swap = new HyperSwapV3Router01();
...
module.exports = { HyperSwapV3Router01 };