...
class HyperSwapV2 {
  ...
  async getQuote(tokenInSymbol, tokenOutSymbol, amountIn) {
    try {
      ...
      const router = new ethers.Contract(this.config.router, this.routerABI, this.provider);
      ...
      console.log(`   出力量: ${ethers.utils.formatUnits(amountOut, 18)}`);
      ...
      return {
        ...
        path,
        rate: parseFloat(ethers.utils.formatUnits(amountOut, 18)) / parseFloat(ethers.utils.formatUnits(amountIn, 18))
      };
    } catch (error) {
      return {
        success: false,
  ...
  async ensureApproval(wallet, tokenSymbol, amount) {
    try {
      ...
      // 現在のAllowance確認
      const currentAllowance = await token.allowance(wallet.address, this.config.router);
  ...
  async swap(tokenInSymbol, tokenOutSymbol, amountIn, slippagePercent = 0.5) {
    try {
      ...
      console.log(`🔄 V2スワップ開始: ${tokenInSymbol} → ${tokenOutSymbol}`);
      ...
      console.log(`   期待出力: ${ethers.utils.formatUnits(expectedOut, 18)} ${tokenOutSymbol}`);
      ...
      // 4. スワップ実行
      console.log('\n🚀 スワップ実行:');
      const router = new ethers.Contract(this.config.router, this.routerABI, wallet);
...
`);
  ...
  try {
    ...
    if (quoteOnly) {
      ...
      if (quote.success) {
        console.log(`✅ レート: 1 ${tokenIn} = ${quote.rate.toFixed(6)} ${tokenOut}`);
        console.log(`   ${amount} ${tokenIn} → ${ethers.utils.formatUnits(quote.amountOut, 18)} ${tokenOut}`);
      } else {
        console.log(`❌ レート取得失敗: ${quote.error}`);
      }
    } else {
      const result = await swap.swap(tokenIn, tokenOut, amountIn, slippage);
