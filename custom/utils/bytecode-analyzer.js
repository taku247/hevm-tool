...
class BytecodeAnalyzer {
    ...
    generateQuoterV2Possibilities() {
        ...
        // HyperSwap固有の可能性
        possibilities.push("quoteExactInputSingle(address,address,uint24,uint256,uint160,bytes)");
        possibilities.push("swapExactInput(bytes,uint256,uint256)");
        possibilities.push("getQuote(address,address,uint24,uint256)");
    ...
    analyzePatterns(bytecode) {
        ...
        // 関数ディスパッチャのパターン検索
        const dispatcherPattern = /80632f80bb1d|8063bd21704a|8063cdca1753|8063f7729d43/gi;
        const dispatcherMatches = [];
        let dispatchMatch;
        while ((dispatchMatch = dispatcherPattern.exec(bytecode)) !== null) {
            dispatcherMatches.push(dispatchMatch[0]);
        }
        
        if (dispatcherMatches.length > 0) {
            console.log(`   QuoterV2関数パターン検出: ${dispatcherMatches.length}個`);
        }
    }
     ...
     * 複数のコントラクトを比較分析
     */
    async compareContracts(addresses) {
            ...
            try {
                ...
                signatures.forEach(sig => {
                    if (sig.signature) {
                        ...
                        console.log(`   ${sig.selector}: 未知の関数`);
                    }
                });
            } catch (error) {
                console.error(`   エラー: ${error.message}`);
        ...
        for (let i = 0; i < names.length - 1; i++) {
            for (let j = i + 1; j < names.length; j++) {
                ...
                if (result1 && result2) {
                    const sigs1 = new Set(result1.map(s => s.selector));
                    const sigs2 = new Set(result2.map(s => s.selector));
...
async function main() {
    ...
    if (args.length === 0) {
        console.log("使用方法:");
        console.log("  node bytecode-analyzer.js <address> [--rpc <url>]");
        console.log("  node bytecode-analyzer.js --compare");
        console.log("\n例:");
        console.log("  node bytecode-analyzer.js 0x03A918028f22D9E1473B7959C927AD7425A45C7C");
        console.log("  node bytecode-analyzer.js --compare");
        return;
    }
